...
class RegisterSession(YowsupApp):
    ...
    def login(self, password=""):
        ...
        self.backend.handleBuddyChanged(
            self.user, "bot", "bot", ["Admin"], Spectrum2.protocol_pb2.STATUS_ONLINE
        )
        self.backend.handleMessage(self.user, "bot", "Please enter your country code")
        ...
        if buddy == "bot" and self.state == self.WANT_CC:
            try:
                ...
                self.backend.handleMessage(
                    self.user, "bot", "Country code must be a number"
                )
                ...
                if country_code != self.number[: len(country_code)]:
                    self.backend.handleMessage(
                        self.user,
                        "bot",
                        "Number does not start with provided country code",
                    )
                else:
            ...
            if self._checkSMSFormat(code):
                ...
                self.backend.handleMessage(
                    self.user, "bot", "Invalid code. Must be of the form XXX-XXX."
                )
        else:
            self.logger.warn(
                "Unauthorised user (%s) attempting to send messages" % self.user
            )
            self.backend.handleMessage(
                self.user,
                buddy,
